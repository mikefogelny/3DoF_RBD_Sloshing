...
def simulate(enable_slosh=True, enable_magnetorquer=True, enable_wheels=True,
    ...
    Parameters
    ----------
    enable_slosh        : bool — include propellant-slosh disturbance torque
    enable_magnetorquer : bool — include magnetic momentum-dumping torque
    enable_wheels       : bool — include reaction-wheel dynamics.  If False,
                                 the PD output drives the body directly as an
                                 ideal torque actuator (no wheel momentum
                                 accumulates, no reaction torque, magnetorquer
                                 has nothing to dump).
    method              : solve_ivp integrator ('RK45', 'DOP853', 'LSODA', …)
    rtol, atol          : solver tolerances
    max_step            : upper bound on solver step size (s)
    """
    # ---- Spacecraft inertia (M&C Ex 7.1) ----
    #Ixx, Iyy, Izz = 10000.0, 9000.0, 12000.0   # principal moments
    #Ixy, Ixz, Iyz =   0.0,  0.0,   0.0   # products of inertia
    
    # ---- Spacecraft inertia (M&C Ex 7.2) ----
    Ixx, Iyy, Izz = 6400.0, 4730.0, 8160.0   # principal moments
    Ixy, Ixz, Iyz =   76.4,   25.6,   40.0   # products of inertia
    ...
    # Spin axes tilted beta = acos(1/sqrt(3)) from +Z, equally spaced 90° in azimuth.
    ...
    # ---- Control gains and saturation limits ----
    #Kp, Kd  = 50.0, 500.0 # M&C Example 7.1
    Kp, Kd  = 10.0, 150.0 # M&C Example 7.2
    u_max   = 1000.0      # N·m   — per-axis PD body-torque saturation
    K_dump  = 1.0        # 1/s   — magnetic momentum-dump gain
    m_max   = 500.0      # A·m^2 — per-axis magnetorquer dipole saturation

    # ---- Slosh model parameters: f(t) = nom + amp * sin(omega * t) ----
    # Set 'amp' = 0 for a constant coefficient.
    slosh_params = {
    ...
    # Slosh initial conditions: column 0 = Ts(0), column 1 = Ts_dot(0)
    slosh_IC = np.zeros((3, 2))

    # ---- Initial conditions ----
    # ---- Initial conditions M&C Example 7.1 ----
    #roll0, pitch0, yaw0 = 112.6, 114.26, 25.16
    #q0    = euler321_to_quat(roll0, pitch0, yaw0)
    #q0    = np.array([0.6853, 0.6953, 0.1531, 0.1531])   
    #w0    = np.array([0.53, 0.53, 0.053])                # degress in the Example
    #w0    = np.radians(w0)                               # body rate, rad/s
    
    # ---- Initial conditions M&C Example 7.2 ----
    #roll0, pitch0, yaw0 = 112.6, 114.26, 25.16
    q0     = (math.sqrt(2) / 2) * np.array([1, 0 , 0, 1])
    w0    = np.array([0.01, 0.01, 0.01])           # body rate, rad/s
    h_w0  = np.zeros(n_w)                               # wheels at rest
    Ts0   = slosh_IC[:, 0].copy()
    Tsd0  = slosh_IC[:, 1].copy()
    q_des = np.array([0.0, 0.0, 0.0, 1.0])              # target = identity
    y0    = pack_state(q0, w0, h_w0, Ts0, Tsd0)

    # ---- Output time grid ----
#    t_end, dt_eval = 300.0, 0.05   # M&C Example 7.1
    t_end, dt_eval = 20*60, 0.05   # M&C Example 7.2
    t_eval = np.arange(0.0, t_end + dt_eval, dt_eval)

    # ---- Parameter bundle for the RHS closure ----
    params = {
        'J': J, 'W': W, 'W_pinv': W_pinv, 'n_w': n_w,
        'Kp': Kp, 'Kd': Kd, 'u_max': u_max, 'q_des': q_des,
        'K_dump': K_dump, 'm_max': m_max,
        'slosh_params': slosh_params,
        'enable_slosh': enable_slosh,
        'enable_magnetorquer': enable_magnetorquer,
    ...
    for k in range(N):
        ...
        if enable_magnetorquer:
            B_body  = magnetic_field_body(t[k])
            m_cmd   = bdot_momentum_dump(W @ h_w, B_body, K_dump, m_max)
            tau_mag = np.cross(m_cmd, B_body)
        else:
            B_body, m_cmd, tau_mag = np.zeros(3), np.zeros(3), np.zeros(3)
        u_cmd, dq = pd_controller(q, w, q_des, Kp, Kd, u_max)
        tau_w = allocate_wheel_torque(u_cmd, W_pinv) if enable_wheels else np.zeros(n_w)
...
def plot_results(t, q_hist, dq_hist, w_hist, u_hist,
    ...
    ax[3].legend([r"$u_x$", r"$u_y$", r"$u_z$"], loc="right")
    ax[3].grid(True)
...
if __name__ == "__main__":
    # Toggle slosh / magnetorquer / wheels here.  Set any to False to disable.
    # With enable_wheels=False the PD output drives the body as an ideal
    # torque actuator (the original pre-Sec. 7.2 form of the simulation).
    (t, q_hist, dq_hist, w_hist, u_hist,
     hw_hist, tauw_hist, m_hist, tmag_hist, B_hist, Ts_hist) = simulate(
        enable_slosh=False, enable_magnetorquer=False,enable_wheels=True)
    plot_results(t, q_hist, dq_hist, w_hist, u_hist,
                 hw_hist, tauw_hist, m_hist, tmag_hist, B_hist, Ts_hist)
